...
import { SentimentData, ManualAnnotation } from "../types";
...
const SentimentChart: React.FC<SentimentChartProps> = ({
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      const activeSentiments = payload
        .filter(
          (item: any) => item.value > 0 && visibleSentiments.has(item.dataKey)
        )
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-white text-sm">
                      {
                        sentimentLabels[
                          item.dataKey as keyof typeof sentimentLabels
                        ]
                      }
                    </span>
                  </div>
  ...
  const getVisibleSentiments = () => {
    if (selectedSentiment) {
      return visibleSentiments.has(selectedSentiment)
        ? [selectedSentiment]
        : [];
    }
    return sentimentTypes.filter((sentiment) =>
      visibleSentiments.has(sentiment)
    );
  };

  // Create annotation areas for intervals
  const annotationAreas = annotations
    .filter((annotation) => annotation.type === "interval")
    .map((annotation) => ({
      start: annotation.startTimestamp!,
      end: annotation.endTimestamp!,
      ...
      <div className="bg-dark-800 border border-dark-600 rounded-xl p-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-white">
            Sentiment Lines & Manual Annotations
          </h3>
          <button
        ...
        <div className="mt-3 text-xs text-gray-400">
          {visibleSentiments.size} of {sentimentTypes.length} sentiment lines
          visible
          {selectedSentiment && (
      ...
      <div className="w-full h-96 bg-dark-800 rounded-xl p-6 border border-dark-600">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            {/* Manual annotation moment markers */}
            {annotations
              .filter((annotation) => annotation.type === "moment")
              .map((annotation, index) => (
                <ReferenceLine
                  key={`annotation-moment-${index}`}
            ...
            {getVisibleSentiments().map((sentiment) => (
              <Line
                ...
                connectNulls={false}
                opacity={
                  selectedSentiment && selectedSentiment !== sentiment ? 0.3 : 1
                }
              />
            ))}
          </LineChart>
