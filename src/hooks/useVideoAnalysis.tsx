...
export const useVideoAnalysis = () => {
  ...
  const uploadToServer = useCallback(
    async (
      file: File
    ): Promise<{ videoId: string; filename: string; duration: number }> => {
    ...
    async (videoId: string): Promise<string> => {
      try {
        const response = await axios.post(
          `${API_BASE_URL}/upload/${videoId}/process`
        );
  ...
  const getAnalysisResults = useCallback(
    async (videoId: string): Promise<VideoAnalysis> => {
      try {
        const response = await axios.get(
          `${API_BASE_URL}/upload/${videoId}/results`
        );
        return {
          ...response.data,
